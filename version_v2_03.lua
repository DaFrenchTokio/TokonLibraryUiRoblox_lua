...
function lib:SpawnMain(title_spawnmain, version_spawnmain)
    ...
    function functions:SpawnTextBox(text, category, execute_text, function_main)
        function_main = function_main or function() end
        local lib_textbox = {
        ...
        if lib_textbox.category then
            ...
            lib_textbox.TextBox.MouseButton1Click:Connect(function()
                ...
                local anim = {
                    anim_1 = game:GetService('TweenService'):Create(lib_notification_textbox.Frame, TweenInfo.new(2), {
                        Size = UDim2.new(0, 250, 0, 250)
                    }),
                    anim_2 = game:GetService('TweenService'):Create(lib_notification_textbox.Frame, TweenInfo.new(1.5), {
                ...
                lib_notification_textbox.Execute.MouseButton1Click:Connect(function()
                    pcall(function_main, lib_notification_textbox.TextBox.Text)
                end)
            end)
        else
            print('Error | No category named '..category)
        ...
        game.Players.LocalPlayer:GetMouse().KeyDown:Connect(function(key)
            if key:lower() == keyname then
                if lib_instance.main.Visible == false then
                    ...
                    lib_instance.main:TweenPosition(UDim2.new(lib_instance.main.Position.X.Scale, lib_instance.main.Position.X.Offset, 0.25, lib_instance.main.Position.Y.Offset))
                else
                    lib_instance.main:TweenPosition(UDim2.new(lib_instance.main.Position.X.Scale, lib_instance.main.Position.X.Offset, 2, lib_instance.main.Position.Y.Offset))
